...
export default function BlogPost({ post, lang = 'en' }) {
            ...
            <div className="blog-container">
                <div className="blog-card p-8">
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        ...
                        >
                            {postLang === 'en' ? 'Switch to Hindi' : 'अंग्रेजी में बदलें'}
                        </button>
                    </div>
                    <h2 className="font-bold text-gray-900" style={{ fontSize: '32px', marginBottom: '20px' }}>{postLang === 'hi' ? post.hindiTitle : post.title}</h2>
                    <div style={{ marginBottom: '0px' }}>
                        <div style={{ color: '#16a34a', marginBottom: '4px' }}>{postLang === 'hi' ? 'द्वारा' : 'By'} {postLang === 'hi' ? (post.hindiAuthor || 'अमितेश गुप्ता') : (post.author || 'Amitesh Gupta')}</div>
                        <div style={{ color: '#4f46e5' }}>{post.getDate ? post.getDate(postLang) : 'August 13, 2025, 04:27 PM IST'}</div>
                    </div>
                    {post.image && (
